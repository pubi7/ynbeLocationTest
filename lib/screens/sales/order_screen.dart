...
class _OrderScreenState extends State<OrderScreen> {
  ...
  Future<void> _submitOrder() async {
    ...
    final canDirectPrint = authProvider.userRole != 'order';

    try {
      // Send order to warehouse backend so it shows on web site
      if (warehouseProvider.connected) {
        // Refresh latest product stocks before sending (avoid oversell when multiple users order simultaneously)
        try {
        ...
        // Validate stock locally (best-effort). Server will still be the source of truth.
        if (mounted) {
          final productProvider =
              Provider.of<ProductProvider>(context, listen: false);
          for (final it in _orderItems) {
            ...
            if (stock > 0 && it.quantity > stock) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    'Үлдэгдэл хүрэлцэхгүй: ${it.productName} (үлдэгдэл $stock, хүссэн ${it.quantity})',
                  ),
      ...
      if (mounted) {
        ...
        await showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            actions: [
              if (canDirectPrint) ...[
                // 🖨️ Хэвлэх товч (BT холбоотой бол шууд хэвлэнэ)
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () async {
                      ...
                      if (!connected) {
                        if (ctx.mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                  '⚠️ Bluetooth принтер холбогдоогүй байна.\nТохиргоо → Bluetooth Принтер хэсгээс холбоно уу.'),
                              backgroundColor: Colors.orange,
                      ...
                      final printed =
                          await btPrinter.printOrderReceipt(savedOrder);
                      if (mounted) {
                        if (printed) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
  ...
  Future<void> _printReceipt() async {
    ...
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    if (isAgentRole(authProvider.userRole) || isManagerRole(authProvider.userRole)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
